...
path <- "~/Dropbox (MN)/Per/PhD/Projects/DNAmeth/hNES DNMT1 KO/RNAseq/PairedParam/Quant/"
...
#### 4.1 Make custom diffex analysis
```{r}
...
## in the following examples, when using the test data, we access it like above
```
...
maPlot(test = dnmt$test$KOvsWT,"DNMT1 KO","CTR",l = .5,p = .005)

# set id = T to label points
maPlot(test = dnmt$test$KOvsWT,"DNMT1 KO","CTR",l = .5,p = .005,id=T)
...
mostSignificantHeat(data = assay(dnmt$VST),test = dnmt$test$KOvsWT,ntop = 70,a1 = dnmt$colData$condition,n1 = "Treatment",a2 = colnames(dnmt$rawCounts),n2 = "sampleNames")
