...
deseqAbs <- R6Class("deseqAbs",
                    ...
                    meanBar = function(genes,cond=NULL,rpkm=FALSE) {
                      ...
                      if ( is.null(cond) ) {
                        data <- ifelse(rpkm,self$rpkmMean$Mean,self$baseMean$Mean)
                        mycolors <- cols(length(unique(self$colData$condition)))
                        padj.a <- self$test$Default$padj
                        names(padj.a) <- rownames(self$test$Default)
                      } else {
                        data <- ifelse(rpkm,self$rpkmMean$Mean[,cond],self$baseMean$Mean[,cond])
