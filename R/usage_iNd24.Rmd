...
newnames <- gsub(pattern = "Aligned.sortedByCoord.out.bam",replacement = "",gsub(pattern = "X.projects.fs1.medpvb.backup.projects.iN.d24.Aligned_hg38_STAR_mMap.hg38.mMap.",replacement = "",x = cnames))
...
## by default, fullAuto do blind dispersion estimates, which is NOT good if many genes are expected to have very different counts in different conditions.
## it might be better to do non-blind estimates, which can be manually done by calling makeVST() with blind = F.
iN$makeVST(blind=F)
...
# maPlot with the RESTi, identify genes (click on points you want to label, then ESC)
maPlot(test = iN$test$RESTi,c1 = "pBpA+RESTi",c2 = "pBpA only",l = 1,p = .001)
...
# generate volcanoplot RESTi, identify genes in plot (click on points you want to label, then ESC)
volcanoPlot(test = iN$test$RESTi,id = T)
