                     ...
                     read_file = function(filename,filter=5) {
                       ...
                       if(!is.na(filename)) {
                         ...
                         if(file.exists(filtered.file)) {
                           cat(">Info: You have previously read and filtered this count file on",filter,"reads, so the filtered file will be read. If you do not want to read it, and the raw file instead, please delete the filtered file, or specify another filtering cutoff!\n")
                           cat(">>Reading filtered featureCount file (that was previously filtered on",filter,"reads).\n")
